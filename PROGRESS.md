...
| 6 | 打包 APK（Android Studio Gradle + Chaquopy 集成） | ⏳ 未开始 |
...
- `requirements_android.txt`：Android 兼容依赖清单（新增 `anthropic`）。
- 业务逻辑（35+ API 端点）完全不变。
...
- 纯 HTML/CSS 静态设计稿：Tailwind CDN + Lucide 图标，苹果风格配色（`colors_and_type.css`）。
...
- **Anthropic API 国内直连超时**：`api.anthropic.com` 国内网络不稳定，LLM 模式需要代理或依赖 fallback 模式。
